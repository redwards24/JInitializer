...
public class TimeSelector extends JInitializer<JPanel, TimeSelector>
{
	...
	private void draw()
	{
		get().setLayout(new GridBagLayout());
		...
		get().add(  hourLabel, grid.set(  x, y));
		get().add(hourSpinner, grid.set(++x, y));
		
		get().add(minuteLabel  , grid.set(--x, ++y));
		get().add(minuteSpinner, grid.set(++x, y));
		
		grid.setAnchor(Grid.EAST);
		get().add(meridiemBar, grid.set(x, ++y));
	}
